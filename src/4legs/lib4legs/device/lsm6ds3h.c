...
static LSM6DS3H_DATA_INTERNAL sRxBuf;
static LSM6DS3H_DATA sImuData;
static LSM6DS3H_CAPTURED_CB sCapturedDoneCb = NULL;
...
int lsm6ds3h_probe(void)
{
	...
	ret = samd51_i2c_txrx(SAMD51_SERCOM0, LSM6DS3H_ADR, txbuf, 1, rxbuf, 1, NULL);
	if (ret != 0) {
		return ret;
	}
	...
	txbuf[1] = ctrl3_c;
	ret = samd51_i2c_txrx(SAMD51_SERCOM0, LSM6DS3H_ADR, txbuf, 2, NULL, 0, NULL);
	if (ret != 0) {
		return ret;
	}
...
static void _checkDataReadyAndRead(void)
{
	...
	if ((sAccelReady != 0) && (sGyroReady != 0)) {
		...
		while ((ret != 0) && (ret != AI_ERROR_I2C_BUSY)) {
			ret = samd51_i2c_txrx(SAMD51_SERCOM0, LSM6DS3H_ADR, sTxbuf, 1, sRxBuf.bytes, 12, _capture_done_cb);
		}
	}
...
static void _capture_done_cb(int status)
{
	if (status == AI_OK && sCapturedDoneCb != NULL) {
		sImuData.accel[0] = (float)sRxBuf.imu.accel[0] * LSM6DS3H_ACCEL_LSB;
		sImuData.accel[1] = (float)sRxBuf.imu.accel[1] * LSM6DS3H_ACCEL_LSB;
		sImuData.accel[2] = (float)sRxBuf.imu.accel[2] * LSM6DS3H_ACCEL_LSB;

		sImuData.gyro[0] = (float)sRxBuf.imu.gyro[0] * LSM6DS3H_ANGUL_LSB;
		sImuData.gyro[1] = (float)sRxBuf.imu.gyro[1] * LSM6DS3H_ANGUL_LSB;
		sImuData.gyro[2] = (float)sRxBuf.imu.gyro[2] * LSM6DS3H_ANGUL_LSB;
		
		sCapturedDoneCb(&sImuData);
	}
