...
/*---------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------*/
uint8_t libxmegaE5_spi_master_initialize(SPI_TYPE type, SPI_TRANSFER_MODE mode, SPI_DATA_ORDER order, SPI_CLK_SELECT clk)
{
	...
	case SPI_CLK_DIV_128:
		...
		break;
	default:
		return LIB_XMEGA_E5_ERROR_SPI_INVALID_CLK;
	}
	...
	case SPI_TRANSFER_MODE_3:
		...
		break;
	default:
		return LIB_XMEGA_E5_ERROR_SPI_INVALID_MODE;
	}
	...
	case SPI_DATA_ORDER_MSB_FIRST:
		...
		break;
	default:
		return LIB_XMEGA_E5_ERROR_SPI_INVALID_ORDER;
	}
	...
	case SPI_TYPE_4WIRE:
		...
		break;
	default:
		return LIB_XMEGA_E5_ERROR_SPI_INVALID_TYPE;
	}
...
/*---------------------------------------------------------------------------*/
uint8_t libxmegaE5_spi_tx(uint8_t data)
{
/*
	uint8_t input = 0xff;
	libxmegaE5_gpio_input(GPIO_PORTC, GPIO_PIN4, &input);
	if (input) {
		spi_txrx_pio(data);
		return LIB_XMEGA_E5_ERROR_OK;
	}
	
	libxmegaE5_gpio_output(GPIO_PORTD, GPIO_PIN1, 0);
*/
	//J データ書込み -> IF ビットが落ちる
	SPIC.DATA = data;
...
/*---------------------------------------------------------------------------*/
uint8_t libxmegaE5_spi_rx(void)
{
/*
	uint8_t input = 0xff;
	libxmegaE5_gpio_input(GPIO_PORTC, GPIO_PIN4, &input);
...
/*---------------------------------------------------------------------------*/
uint8_t libxmegaE5_spi_txrx_pio(uint8_t data)
{
	uint8_t ctrl = SPIC.CTRL;
	uint8_t ret = 0;
	uint8_t input = 0xff;
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 0);

	SPIC.CTRL = 0;

	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN7, (data & 0x80));
	libxmegaE5_gpio_input (GPIO_PORTC, GPIO_PIN6, &input);
	if (input) ret |= 0x80;
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 1);
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 0);
	
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN7, (data & 0x40));
	libxmegaE5_gpio_input (GPIO_PORTC, GPIO_PIN6, &input);
	if (input) ret |= 0x40;
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 1);
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 0);

	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN7, (data & 0x20));
	libxmegaE5_gpio_input (GPIO_PORTC, GPIO_PIN6, &input);
	if (input) ret |= 0x20;
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 1);
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 0);

	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN7, (data & 0x10));
	libxmegaE5_gpio_input (GPIO_PORTC, GPIO_PIN6, &input);
	if (input) ret |= 0x10;
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 1);
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 0);

	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN7, (data & 0x08));
	libxmegaE5_gpio_input (GPIO_PORTC, GPIO_PIN6, &input);
	if (input) ret |= 0x08;
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 1);
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 0);

	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN7, (data & 0x04));
	libxmegaE5_gpio_input (GPIO_PORTC, GPIO_PIN6, &input);
	if (input) ret |= 0x04;
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 1);
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 0);

	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN7, (data & 0x02));
	libxmegaE5_gpio_input (GPIO_PORTC, GPIO_PIN6, &input);
	if (input) ret |= 0x02;
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 1);
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 0);

	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN7, (data & 0x01));
	libxmegaE5_gpio_input (GPIO_PORTC, GPIO_PIN6, &input);
	if (input) ret |= 0x01;
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 1);
	libxmegaE5_gpio_output(GPIO_PORTC, GPIO_PIN5, 0);
